...
class AddGroupPermissionGUI(val plugin: SillyPlugin) {

    fun open(player: Player, groupName: String, page: Int = 0, sortDescending: Boolean = false, searchQuery: String? = null) {
        ...
        player.openGui(title, 6) {
            ...
            for (i in start until end) {
                ...
                setItem(slots[i - start], item(Material.PAPER) {
                    ...
                    lore(
                        ...
                        "",
                        " <gray>» <green>Click <gray>to add to $groupName",
                        ""
                    )
                }) {
                    plugin.userManager.database.addGroupPermission(groupName, perm)
                    player.msg("<green>Added permission '$perm' to group $groupName.")
                    plugin.groupDetailGUI.open(player, groupName)
                }
            }

            setItem(54, item(Material.COMPASS) {
            ...
            setItem(45, item(Material.COMPARATOR) {
                ...
                open(player, groupName, 0, !sortDescending, searchQuery)
            }

            // Back button
            setItem(46, item(Material.DARK_OAK_DOOR) {
                name("<red><bold>Back")
                lore(" <gray>» Return to $groupName's perms")
            }) {
                plugin.groupDetailGUI.open(player, groupName)
            }
            ...
            setItem(47, item(Material.WRITABLE_BOOK) {
                ...
                AnvilInputUI.open(player, "Custom Permission", "Enter perm...") { input ->
                    if (!input.equals("cancel", true) && input.isNotBlank() && input != "Enter perm...") {
                        plugin.userManager.database.addGroupPermission(groupName, input)
                        player.msg("<green>Added custom permission '$input' to group $groupName.")
                        plugin.groupDetailGUI.open(player, groupName)
                    } else {
                        open(player, groupName, page, sortDescending, searchQuery)
                    }
                }
            }
            ...
            if (page > 0) {
                ...
                }) { open(player, groupName, page - 1, sortDescending, searchQuery) }
            }
